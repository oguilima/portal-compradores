...
export class AvalicaoPropostasComponent {
  ...
  technicalOpinion(line: any) {
    this.parecer.open();
  }

  openOptions(line: any) {

  }

  openRequest(line: any) {

  }

  manageProposal() {
    this.gerenciarProposta.open();
  }

  notifySupplierMail() {
    this.utilsService.showSweetAlert({ title: "Sucesso!", icon: "success", message: "Fornecedor notificado com sucesso!", reload: false });
  }

  save: PoModalAction = {
    action: () => {
      this.utilsService.showSweetAlert({ title: "Sucesso!", icon: "success", message: "Dados alterados com sucesso!", reload: false });
      this.gerenciarProposta.close();
    },
    label: 'Salvar'
  };
